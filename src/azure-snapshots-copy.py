...
class AzureSnapshotManager:
    ...
    def copy_snapshots(self):
        ...
        logger.info(
            f"Fetching disk snapshots in resource group '{self.resource_group}' to copy..."
        )
        snapshots = list(
            self.compute_client.snapshots.list_by_resource_group(self.resource_group)
        )
        ...
        for snapshot in snapshots:
            try:
                if (
                    snapshot.tags["CrossRegionCopy"]
                    == self.default_source_snapshot_tags["CrossRegionCopy"]
                ):
                    logger.info(
                        f"Snapshot '{snapshot.name}' already copied to {snapshot.tags['CopiedRegion']}"
                    )
                    continue
            ...
            logger.info(
                f"Begin incremental copy '{snapshot.name}' to resource group "
            ...
            if poll_result.provisioning_state == "Succeeded":
                logger.info(
                    f"Provisioned snapshot '{poll_result.name}' in region '{poll_result.location}'"
                )
                self._add_tags(snapshot.name)
            else:
                logger.warn(
                    f"Provisioning state for copy snapshot '{poll_result.name}' is "
    ...
    def delete_snapshots(self):
        ...
        logger.info(
            f"Fetching disk snapshots in resource group '{self.destination_resource_group}' to delete..."
        )
        snapshots = self.compute_client.snapshots.list_by_resource_group(
        ...
        for snapshot in snapshots:
            if snapshot.time_created <= past_time:
                logger.info(
                    f"Begin delete snapshot '{snapshot.name}' in "
                ...
                self.compute_client.snapshots.begin_delete(
                    self.destination_resource_group, snapshot.name
                )
            else:
                logger.info(
                    f"Snapshot '{snapshot.name}' is not expired, created on "
    ...
    def _add_tags(self, snapshot_name):
        ...
        if poll_result.provisioning_state == "Succeeded":
            ...
            logger.warn(
                f"Provisioning stage for update tags on snapshot '{snapshot_name}' is"
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Azure Snapshot Manager",
    ...
    parser.add_argument(
        ...
        envvar="DESTINATION_RESOURCE_GROUP",
        required=True,
        action=EnvDefault,
    ...
    parser.add_argument(
        ...
        action=EnvDefault,
        required=True,
        help="Destination Azure region for copy operation",
    )
